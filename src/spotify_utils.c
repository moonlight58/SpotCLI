...
void spotify_print_playlist_full(SpotifyPlaylistFull *playlist) {
    if (!playlist) {
        printf("No playlist information available\n");
        return;
    }
    ...
    printf("║ Owner: %s\n", 
           strlen(playlist->owner_name) > 0 ? playlist->owner_name : playlist->owner_id);
    ...
    if (playlist->tracks_count > 0) {
        printf("╠════════════════════════════════════════════════════════════════╣\n");
        printf("║ 🎵 TRACKS (%d loaded):\n", playlist->tracks_count);
        printf("║\n");
        
        for (int i = 0; i < playlist->tracks_count && i < 20; i++) {
            SpotifyTrack *t = &playlist->tracks[i];
            int min = t->duration_ms / 60000;
            int sec = (t->duration_ms / 1000) % 60;
